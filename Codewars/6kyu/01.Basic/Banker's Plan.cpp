// https://www.codewars.com/kata/56445c4755d0e45b8c00010a/train/cpp
...
public:
    static bool fortune(int f0, double p, int c0, int n, double i) {
      for (int year = 0; year < n - 1; year++) {
        c0 = (year == 0) ? c0 : static_cast<int>(std::floor(c0 + (c0 * i / 100)));
        f0 = f0 + static_cast<int>(std::floor(f0 * p / 100));
        f0 = (f0 - c0);
        if (f0 < 0) {
            return false;
        }
